Feature: move files
  ...
  Scenario: cancel moving a file
    Given user "Alice" has logged in using the webUI
    And the user has browsed to the files page
    When the user opens the file action menu of folder "lorem.txt" using the webUI
    And the user selects move action for folder "lorem.txt" using the webUI
  ...
  Scenario: sharee moves a file shared by sharer into another folder
    ...
    And user "Brian" has created file "testFile.txt"
